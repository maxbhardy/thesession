...
class TheSessionModel(torch.nn.Module):
    ...
    def __init__(self, enable_fusion: bool = True, device: str | None = None, **kwargs):
        ...
        self.clap_model = CLAP_Module(enable_fusion=enable_fusion, device=self.device, **kwargs)

    def forward(self, X: torch.Tensor) -> torch.Tensor:
        return self.clap_model.get_audio_embedding_from_data(X, use_tensor=True)
    
    def load_ckpt(self, ckpt: str | None = None, model_id: int = -1, verbose: bool = True):
    ...
    def toggle_gradients(self, requires_grad: bool = False, parameters: list[str] | None = None):
